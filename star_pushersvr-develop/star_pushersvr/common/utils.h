...
public:
    ...
    inline const char* devid2str(uint64_t devid) {
        ...
        {
            char buf[32];
            m_devIdB2SMap[devid] = ::devid2str(devid, buf, sizeof(buf));
    ...
    inline uint64_t str2devid(const std::string &devid) {
        ...
        {
            uint64_t bDevId = ::str2devid(devid.c_str());
    ...
    inline const char* customerId2Str(uint32_t customerId) {
        ...
        {
            char buf[32];
            m_customerIdMap[customerId] = ::customerId2Str(customerId, buf, sizeof(buf));
