...
// 使用poll代替select，避免接口调用受限于FD_SETSIZE大小, 2023.01.09, Bill
#ifndef CPPHTTPLIB_USE_POLL
#define CPPHTTPLIB_USE_POLL
#endif // CPPHTTPLIB_USE_POLL
...
using Range = std::pair<ssize_th, ssize_th>;
...
public:
  ...
  virtual ssize_th read(char *ptr, size_t size) = 0;
  virtual ssize_th write(const char *ptr, size_t size) = 0;
  virtual void get_remote_ip_and_port(std::string &ip, int &port) const = 0;
  virtual socket_t socket() const = 0;

  template <typename... Args>
  ssize_th write_format(const char *fmt, const Args &...args);
  ssize_th write(const char *ptr);
  ssize_th write(const std::string &s);
};
...
inline uint64_t get_header_value<uint64_t>(const Headers &headers,
  ...
  auto it = rng.first;
  std::advance(it, static_cast<ssize_th>(id));
  if (it != rng.second) {
...
template <typename... Args>
inline ssize_th Stream::write_format(const char *fmt, const Args &...args) {
...
ssize_th send_socket(socket_t sock, const void *ptr, size_t size, int flags);

ssize_th read_socket(socket_t sock, void *ptr, size_t size, int flags);
...
public:
  ...
  ssize_th read(char *ptr, size_t size) override;
  ssize_th write(const char *ptr, size_t size) override;
...
template <typename T> inline ssize_th handle_EINTR(T fn) {
  ssize_th res = false;
  while (true) {
    res = fn();
...
inline ssize_th read_socket(socket_t sock, void *ptr, size_t size, int flags) {
  return handle_EINTR([&]() {
    return recv(sock,
#ifdef _WIN32
...
inline ssize_th send_socket(socket_t sock, const void *ptr, size_t size,
                           int flags) {
  return handle_EINTR([&]() {
    return send(sock,
#ifdef _WIN32
...
inline ssize_th select_read(socket_t sock, time_t sec, time_t usec) {
#ifdef CPPHTTPLIB_USE_POLL
...
inline ssize_th select_write(socket_t sock, time_t sec, time_t usec) {
#ifdef CPPHTTPLIB_USE_POLL
  struct pollfd pfd_read;
  pfd_read.fd = sock;
...
inline ssize_th write_headers(Stream &strm, const Headers &headers) {
  ssize_th write_len = 0;
  ...
  if (std::regex_match(s, m, re_first_range)) {
    ...
    split(&s[pos], &s[pos + len], ',', [&](const char *b, const char *e) {
      ...
      if (std::regex_match(b, e, cm, re_another_range)) {
        ssize_th first = -1;
        if (!cm.str(1).empty()) {
          first = static_cast<ssize_th>(std::stoll(cm.str(1)));
        }

        ssize_th last = -1;
        if (!cm.str(2).empty()) {
          last = static_cast<ssize_th>(std::stoll(cm.str(2)));
        }
...
get_range_offset_and_length(const Request &req, size_t content_length,
  ...
  auto slen = static_cast<ssize_th>(content_length);

  if (r.first == -1) {
    r.first = (std::max)(static_cast<ssize_th>(0), slen - r.second);
    r.second = slen - 1;
  }
...
get_range_offset_and_length(const Request &req, const Response &res,
  ...
  if (r.second == -1) {
    r.second = static_cast<ssize_th>(res.content_length_) - 1;
  }
...
inline ssize_th Stream::write(const char *ptr) {
  return write(ptr, strlen(ptr));
}

inline ssize_th Stream::write(const std::string &s) {
...
inline ssize_th SocketStream::read(char *ptr, size_t size) {
#ifdef _WIN32
  size =
      (std::min)(size, static_cast<size_t>((std::numeric_limits<int>::max)()));
#else
  size = (std::min)(size,
                    static_cast<size_t>((std::numeric_limits<ssize_th>::max)()));
#endif
  ...
  if (read_buff_off_ < read_buff_content_size_) {
    ...
    if (size <= remaining_size) {
      ...
      read_buff_off_ += size;
      return static_cast<ssize_th>(size);
    } else {
      memcpy(ptr, read_buff_.data() + read_buff_off_, remaining_size);
      read_buff_off_ += remaining_size;
      return static_cast<ssize_th>(remaining_size);
    }
  }
  ...
  if (size < read_buff_size_) {
    ...
    if (n <= 0) {
      return n;
    } else if (n <= static_cast<ssize_th>(size)) {
...
inline ssize_th SocketStream::write(const char *ptr, size_t size) {
...
inline ssize_th BufferStream::read(char *ptr, size_t size) {
  ...
  return static_cast<ssize_th>(len_read);
}

inline ssize_th BufferStream::write(const char *ptr, size_t size) {
...
inline ssize_th SSLSocketStream::read(char *ptr, size_t size) {
...
inline ssize_th SSLSocketStream::write(const char *ptr, size_t size) {
  if (is_writable()) {
