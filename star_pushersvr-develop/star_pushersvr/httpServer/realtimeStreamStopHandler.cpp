...
void realtimeStreamStopHandler::post(const Json::Value & jsonReq, Json::Value & jsonResp)
{
    ...
    std::shared_ptr<StreamBase> temp = StreamModInterface::realStreamMng()->getStream(id);
    if (temp == NULL)
    {
    ...
    SPDINFO("[http] stop realtime stream {} , duration: {} ms", id, GetMillisecondCounter() - startTick);
